/* 以下の基本演算を備えた2次元配列を、コンテナを表すクラステンプレートとして作る
    要素へのアクセス
        at(), data()
    容量クエリ
    イテレータ
...
class array2d {
    ...
    public:
        ...
        /*
            ...
            {x, y, z}形式のリテラルを受け取るための特殊な型。
            例えば、
                void f(std::initializer_list<int>)
            と宣言された関数は、以下の記法で呼び出せる。
                f({1, 7, 2, 3});

        */
        explicit array2d(std::initializer_list<T> l): arr(l) {}

        /* 非定数オブジェクトに対して呼び出され、配列のデータを変更することができる。 */
        constexpr T* data() noexcept {return arr.data();}

        /* 定数オブジェクトに対して呼び出され、配列のデータを変更することはできない。 */
